    ...
    this.userService.login(this.form.get('username').value, this.form.get('password').value)
      .subscribe(res => {
          if (res) {
            ...
            this.snackBar.open('Logged in. You will be redirected shortly', '', {duration: 6000, panelClass: 'green-snackbar'});
          }
        }, err => {
          this.isRequestSend = false;
          this.snackBar.open('Invalid username or password', '', {duration: 3000, panelClass: 'red-snackbar'});
        }
      );
  }
