...
export class BankAccountListComponent implements OnInit {
  ...
  // dla comboboxa przy wykonywaniu Transferu
  currencyList: string[];
  ...
  createBankAccount() {
    ...
    if(this.bankAccounts.length >= 5){
      this.fetchBankAccounts(); this.snackBar.open('You have reached your account limit', '', { duration: 3000, panelClass: 'red-snackbar' });
      return;
    }
    this.bankAccountService.create(this.bankAccountForm.value)
      .subscribe(res => {
        this.fetchBankAccounts(); this.snackBar.open('A new account has been created', '', { duration: 3000, panelClass: 'green-snackbar' });
      });
  }
