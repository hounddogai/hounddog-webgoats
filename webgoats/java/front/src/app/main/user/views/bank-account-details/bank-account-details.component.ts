...
export class BankAccountDetailsComponent implements OnInit {
  ...
  chartDatasets: Array<any> = [
    { data: [0, 0, 0, 0, 0, 0], label: 'Balances' }
  ];
    ...
    this.activatedRoute.params.subscribe(params => {
      ...
      this.bankAccountService.findById(this.id)
        .subscribe(res => {
          ...
          this.fillChartData();
          // transactionType to Transfer/ wplata itp
  ...
  deleteBankAccount() {
    ...
    if (confirmNumber) {
      ...
      if (confirmNumber !== this.bankAccount.number) {
        this.snackBar.open('Please enter a valid account number', '', { duration: 3000, panelClass: 'red-snackbar' });
        return;
      }

      if (this.hasPositiveBalance()) {
        this.snackBar.open('At least one balance has funds', '', { duration: 3000, panelClass: 'red-snackbar' });
        return;
      }

      this.bankAccountService.deleteById(this.bankAccount.id).subscribe(res => {
        this.snackBar.open('The account has been Deleted', '', { duration: 3000, panelClass: 'green-snackbar' });
