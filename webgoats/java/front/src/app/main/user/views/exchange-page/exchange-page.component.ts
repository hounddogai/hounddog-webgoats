...
export class ExchangePageComponent implements OnInit {
  ...
  calculate() {
    if (!this.exchangeCurrency.balance || this.exchangeCurrency.balance <= 0) {
      this.snackBar.open('Sum must be greater than 0', '', { duration: 3000, panelClass: 'red-snackbar' });
      return;
    }
  ...
  convert() {
    ...
    this.exchangeCurrencyService.create(this.exchangeCurrency)
      .subscribe(res => this.snackBar.open('Currency converted', '', { duration: 3000, panelClass: 'green-snackbar' }),
