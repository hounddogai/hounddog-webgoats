...
export class CreditListComponent implements OnInit {
  ...
  acceptCredit(id: number) {
    this.creditService.changeStatus(id, 'ACTIVE')
      .subscribe(res => {
        this.fetchCreditList();
        this.snackBar.open('Credit accepted', '', { duration: 3000, panelClass: 'green-snackbar' });
      });
  }
  ...
  rejectCredit(id: number) {
    this.creditService.changeStatus(id, 'CANCELED')
      .subscribe(res => {
        this.fetchCreditList();
        this.snackBar.open('Credit declined', '', { duration: 3000, panelClass: 'blue-snackbar' });
      });
  }
  ...
  changeStatus() {
    ...
    this.creditService.changeStatus(this.credit.id)
      .subscribe(res => {
        this.credit = res;
        this.fetchCreditList();
        this.snackBar.open('Status changed', '', { duration: 3000, panelClass: 'green-snackbar' });
      });
  }
  ...
  searchCredit() {
    ...
    this.creditService.findById(id)
      .subscribe(res => this.credit = res, err => this.snackBar.open('No credit found', '', { duration: 3000, panelClass: 'red-snackbar' }));
  }
