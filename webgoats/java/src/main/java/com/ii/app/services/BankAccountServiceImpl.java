...
public class BankAccountServiceImpl implements BankAccountService {
    ...
    public BankAccountOut findById(Long id) {
        return bankAccountRepository.findById(id)
            .map(bankAccountMapper::entityToDTO)
            .orElseThrow(() -> {
                Optional<User> byIdentifier=Optional.empty();
                Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
                if(authentication != null) {
                    byIdentifier = userRepository.findByIdentifier(authentication.getName());
                }
                return new RuntimeException("Not found for bank account " + id + " for user " + byIdentifier);
            });
    }
